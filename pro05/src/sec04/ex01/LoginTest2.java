...
@WebServlet("/loginTest2")	//http://localhost:8080/pro05/
public class LoginTest2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( id != null && (id.length() != 0) ){	//아이디를 입력했다면
			...
			if(id.equals("admin")){	//관리자계정(admin)으로 입력했다면 관리자 화면으로 응답
				out.print("<html>");
				out.print("<body>");
					out.print("<font size='12'>"+ id + "관리자님, 로그인 하셨습니다.<br>");
					out.print("<input type='button' value='회원정보수정' />");
					out.print("<input type='button' value='회원정보삭제' />");
				out.print("</body>");
				out.print("</html>");
			} else {	//일반 계정으로 입력했다면 일반 사용자 화면으로 응답
				out.print("<html>");
				out.print("<body>");
			...
			out.print("<body>");
				...
				out.print("<a href='http://localhost:8080/pro05/test01/login2.html'> 로그인 페이지로 이동 </a>");
			out.print("</body>");
			out.print("</html>");
		}
